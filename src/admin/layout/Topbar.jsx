...
const Topbar = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  useEffect(() => {
    ...
    loadProfilePic();
    window.addEventListener('profileImageChanged', loadProfilePic);
    return () => window.removeEventListener('profileImageChanged', loadProfilePic);
  }, []);
  ...
  return (
    <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-6 lg:px-10 shrink-0 sticky top-0 z-[50]">
      <div className="flex items-center gap-4">
        {!isSidebarOpen && (
          <button onClick={() => setIsSidebarOpen(true)} className="p-2.5 rounded-xl hover:bg-slate-100 text-slate-500 transition-all active:scale-95">
            <Menu size={20} />
          </button>
        )}
      </div>
      ...
      <div className="flex items-center gap-5">
              ...
              <div className="max-h-[300px] overflow-y-auto custom-scrollbar">
                {notifications.map(n => (
                  <div key={n.id} className="p-4 border-b border-slate-50 hover:bg-slate-50/80 transition-colors cursor-pointer flex gap-4 group">
                    <div className={`w-10 h-10 rounded-[0.8rem] flex items-center justify-center shrink-0 ${n.bg} ${n.color} transition-transform group-hover:scale-110`}>{n.icon}</div>
                    <div>
        ...
        <div className="relative" ref={profileRef}>
          <button 
            ...
            className="w-10 h-10 rounded-[1rem] bg-brand-dark border border-slate-800 flex items-center justify-center font-black text-white text-sm hover:shadow-lg hover:bg-black transition-all focus:outline-none overflow-hidden"
          >
            {profilePic ? <img src={profilePic} alt="Profile" className="w-full h-full object-cover" /> : "A"}
          </button>
          ...
          {isProfileOpen && (
            <div className="absolute top-[3.5rem] right-0 w-64 bg-white rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-slate-100 overflow-hidden z-[500] animate-in fade-in zoom-in-95 duration-200">
              <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-4">
                <div className="w-12 h-12 rounded-[1rem] bg-brand-primary/10 text-brand-primary flex items-center justify-center font-black text-xl overflow-hidden">
                  {profilePic ? <img src={profilePic} alt="Profile" className="w-full h-full object-cover" /> : "A"}
                </div>
                <div>
              ...
              <div className="py-2">
                <button onClick={() => { navigate('/admin/profile'); setIsProfileOpen(false); }} className="w-full flex items-center gap-3 px-5 py-3 text-xs font-bold text-slate-600 hover:bg-brand-primary/5 hover:text-brand-primary transition-colors">
