...
import { 
  ...
  Eye as EyeIcon, ShieldCheck, Zap, HeartHandshake, ArrowUpRight, Plus, Trash2, Loader2 
} from 'lucide-react';
...
const AboutValuesEditor = () => {
  ...
  const [valuesData, setValuesData] = useState({
    ...
    values: [
      { title: "Transparency", desc: "No hidden costs. Detailed quotes provided upfront for total peace of mind.", color: "bg-blue-500", iconName: "EyeIcon" },
      { title: "Safety First", desc: "Every technician is background-verified and strictly follows safety protocols.", color: "bg-emerald-500", iconName: "ShieldCheck" },
      { title: "Speedy Service", desc: "We value your time. Our rapid response team ensures on-time arrival every time.", color: "bg-amber-500", iconName: "Zap" },
      { title: "Quality Care", desc: "Professional handling of your property with 100% satisfaction guarantee.", color: "bg-emerald-500", iconName: "HeartHandshake" }
    ]
  });
  ...
  if (status === 'loading' && !content) {
    return (
      <div className="h-screen flex items-center justify-center font-bold text-slate-400 uppercase tracking-widest text-xs">
        <Loader2 className="animate-spin mr-2" size={16} /> Loading Values Lab...
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans pb-20">
      <nav className="sticky top-0 z-[50] bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between shadow-sm">
        ...
        <button 
          ...
          className="bg-slate-900 text-white px-5 py-2 rounded-full font-extrabold text-xs flex items-center gap-2 hover:bg-blue-600 transition-all shadow-lg shadow-slate-200 disabled:opacity-50"
        >
