...
import {
    ...
    Sparkles, Plus, X, Trash2, Edit3, ArrowRight, Save, RotateCcw,
...
const BlogPageOverview = () => {
    ...
    const [sections, setSections] = useState(() => {
        const saved = localStorage.getItem('tricksy_blog_modules');
        return saved ? JSON.parse(saved) : defaultSections;
    });

    useEffect(() => {
        localStorage.setItem('tricksy_blog_modules', JSON.stringify(sections));
    }, [sections]);
    ...
    const handleRestoreDefaults = () => {
        if (window.confirm("Restore default Blog sections?")) {
    ...
    const openCreateModal = () => {
        setEditingId(null);
        setFormData({ name: '', path: '', status: 'Draft', theme: 'indigo', iconKey: 'box' });
        setIsModalOpen(true);
    };
    ...
    const handleSaveModule = (e) => {
        ...
        const finalPath = formData.path.trim() || `/admin/pages/blog/${formData.name.toLowerCase().replace(/\s+/g, '-')}`;
        const moduleData = {
            name: formData.name,
        ...
        if (editingId) {
            ...
            setSections([...sections, { id: `blog-custom-${Date.now()}`, ...moduleData }]);
        }
        ...
        <div className="min-h-screen bg-[#FDFDFD] p-6 lg:p-12 font-sans relative">
            <div className="w-full mb-10 md:mb-12 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-8">
                <div className="space-y-1">
                    ...
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">Blog Modules</h1>
            ...
            <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
                ...
                <div onClick={openCreateModal} className="group p-4 md:p-5 rounded-[1.5rem] border-2 border-dashed border-slate-200 flex items-center gap-4 text-slate-400 hover:border-indigo-400 hover:text-indigo-500 hover:bg-indigo-50/50 transition-all cursor-pointer min-h-[90px] active:scale-95">
                    ...
                    <div className="flex-1"><h3 className="text-[14px] font-bold text-slate-700 group-hover:text-indigo-600">Add Module</h3></div>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl relative z-10 border border-slate-100 overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-300">
                        <div className="px-8 pt-8 pb-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                            <div>
                                <h2 className="text-2xl font-black text-slate-900">{editingId ? 'Edit Module' : 'Create Blog Module'}</h2>
                                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">Configure your blog section</p>
                            </div>
                            ...
                            <form onSubmit={handleSaveModule} className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="md:col-span-2">
                                        ...
                                        <input required autoFocus value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-2 ring-indigo-100 transition-all" placeholder="e.g. Content Editor" />
                                    </div>
                        ...
                        <div className="px-8 py-5 border-t border-slate-50 flex justify-end gap-4 bg-white">
                            ...
                            <button onClick={handleSaveModule} className="px-8 py-3 rounded-xl text-xs font-black text-white bg-indigo-600 hover:bg-indigo-700 shadow-lg flex items-center gap-2 transition-all">
                                <Save size={18} /> {editingId ? 'UPDATE MODULE' : 'CREATE MODULE'}
                            </button>
                        </div>
                    </div>
                </div>
