...
import { 
  ...
  Type, AlignLeft, Undo, Loader2, UploadCloud, Image as ImageIcon, Globe
} from 'lucide-react';
...
const ContactHeroEditor = () => {
  ...
  const subsectionId = id ? parseInt(id, 10) : 34;
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      if (res.success || res.imageUrl) {
        setFormData(prev => ({ ...prev, bgImage: res.imageUrl }));
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] font-sans overflow-hidden selection:bg-emerald-100">
      ...
      <nav className="sticky top-0 z-[100] bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-3 flex items-center justify-between shadow-sm shrink-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-[12px] font-black italic flex items-center gap-2 uppercase tracking-[0.2em] text-slate-800">
            <Settings2 size={16} className="text-emerald-600" /> Hero <span className="text-emerald-400">Lab</span>
          </h1>
        </div>
        ...
        <button onClick={handleSave} disabled={isSaving || isUploading} className="bg-slate-900 text-white px-8 py-2 rounded-xl font-black text-[10px] tracking-widest hover:bg-emerald-600 transition-all flex items-center gap-2 disabled:opacity-50">
        ...
        {(viewMode === 'edit' || viewMode === 'split') && (
          <div className={`${viewMode === 'split' ? 'w-4/12 border-r border-slate-100' : 'w-full max-w-4xl mx-auto mt-8 border rounded-[2rem]'} bg-white flex flex-col h-full overflow-hidden z-10 transition-all duration-300 shadow-2xl shadow-slate-200/50`}>
            <div className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-6 custom-scrollbar">
              <div>
                <h2 className="text-sm font-black text-slate-900 tracking-tight uppercase mb-1">Hero Content</h2>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Manage Landing Banner</p>
              </div>

              <div className="space-y-5">
                <div className="bg-slate-50/50 p-5 rounded-[2rem] border border-slate-100 space-y-4 shadow-inner">
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block flex items-center gap-1.5"><Type size={12} className="text-emerald-500" /> Badge Text</label>
                    <input type="text" name="badgeText" value={formData.badgeText} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-slate-100 rounded-xl text-xs font-bold text-slate-800 outline-none focus:border-emerald-400 transition-all shadow-sm" />
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Title Part 1</label>
                      <input type="text" name="titlePart1" value={formData.titlePart1} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-slate-100 rounded-xl text-xs font-bold text-slate-800 outline-none focus:border-emerald-400 transition-all shadow-sm" />
                    </div>
                    <div>
                      <label className="text-[9px] font-black text-emerald-600 uppercase tracking-widest ml-1 mb-2 block italic underline">Highlight</label>
                      <input type="text" name="titleAccent" value={formData.titleAccent} onChange={handleChange} className="w-full px-4 py-3 bg-emerald-50/50 border border-emerald-100 rounded-xl text-xs font-black text-emerald-700 outline-none focus:border-emerald-400 transition-all shadow-sm" />
                    </div>
                  </div>

                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block flex items-center gap-1.5"><AlignLeft size={12} className="text-emerald-500" /> Subtitle</label>
                    <textarea name="paragraphText" value={formData.paragraphText} onChange={handleChange} rows="4" className="w-full px-4 py-3 bg-white border border-slate-100 rounded-xl text-xs font-medium text-slate-600 outline-none focus:border-emerald-400 transition-all shadow-sm resize-none leading-relaxed" />
                  </div>
                </div>

                <div className="bg-slate-50/50 p-5 rounded-[2rem] border border-slate-100 space-y-4">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 block flex items-center gap-1.5"><ImageIcon size={12} className="text-emerald-500" /> Background Media</label>
                  <div className="relative group h-44 rounded-2xl overflow-hidden bg-white border-2 border-dashed border-slate-200 flex flex-col items-center justify-center cursor-pointer hover:border-emerald-300 transition-all">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center text-white font-black text-[10px] uppercase tracking-widest">
                       {isUploading ? <Loader2 className="animate-spin" size={20} /> : <><UploadCloud size={20} className="mr-2"/> Change Image</>}
                    </div>
            ...
            <div className="w-full max-w-[1200px] h-full bg-slate-900 rounded-[2.5rem] p-3 shadow-2xl border-[10px] border-slate-800 flex flex-col">
              <div className="flex h-8 bg-slate-900 items-center px-4 gap-1.5 border-b border-slate-800/50 shrink-0">
                 ...
                 <div className="flex-1 text-center flex items-center justify-center gap-1.5">
                    ...
                    <span className="text-[6px] font-bold uppercase tracking-widest text-slate-500">tricksy-contact.io</span>
                 </div>
              </div>

              <div className="flex-1 bg-zinc-950 rounded-xl overflow-hidden relative origin-top">
                 <div className="relative w-full h-full flex flex-col items-center justify-center p-10 text-center">
                    <div className="absolute inset-0">
                        <img src={getImageUrl(formData.bgImage)} className="w-full h-full object-cover" alt="Hero Background" />
                        <div className="absolute inset-0 bg-gradient-to-t from-zinc-950/90 via-zinc-950/80 to-transparent"></div> 
                    </div>
                    
                    <div className="relative z-10 flex flex-col items-center w-full">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/20 mb-6 bg-white/5 backdrop-blur-sm">
                            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                            <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/80">{safeText(formData.badgeText)}</span>
                        </div>

                        <h1 className="text-5xl md:text-7xl font-black text-white tracking-tight mb-6 leading-[1.1]">
                            {safeText(formData.titlePart1)} <br/>
                        ...
                        <p className="text-sm md:text-base text-white/80 font-medium max-w-2xl leading-relaxed">
                            {safeText(formData.paragraphText)}
                        </p>
                    </div>
                 </div>
              </div>
