...
const ServiceWizard = () => {
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] font-sans flex flex-col relative pb-28 overflow-hidden">
            <header className="bg-white border-b border-slate-200 sticky top-0 z-[110] px-6 py-4">
                <div className="max-w-[1800px] mx-auto flex items-center justify-between gap-4">
                    <div className="flex items-center gap-5 shrink-0">
                        <button onClick={() => navigate('/admin/pages/services')} className="p-2.5 bg-slate-50 hover:bg-rose-50 rounded-xl text-slate-400 border border-slate-100"><X size={18} /></button>
                        <div>
                    ...
                    <div className="hidden xl:flex items-center gap-1 bg-slate-100 p-1 rounded-[1.5rem] border border-slate-200/50">
                        {steps.map((step, index) => (
                            <button key={step.id} onClick={() => index <= currentStep && setCurrentStep(index)} className={`flex items-center gap-2 px-5 py-2.5 rounded-[1.1rem] transition-all ${index === currentStep ? 'bg-white shadow-lg text-emerald-600 scale-105' : 'text-slate-400'}`}>
                                <step.icon size={14} />
                    ...
                    {/* <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200/50">
                        {[ {id: 'edit', i: Edit3}, {id: 'split', i: Columns}, {id: 'preview', i: Eye} ].map(m => (
                            // <button key={m.id} onClick={() => setViewMode(m.id)} className={`p-2.5 rounded-xl transition-all ${viewMode === m.id ? 'bg-white shadow-sm text-emerald-600' : 'text-slate-400'}`}>
                            //     <m.i size={16} />
                            // </button>
                        ))}
                    </div> */}
            ...
            <footer className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200 p-5 z-[1000]">
                <div className="max-w-[1800px] mx-auto flex items-center justify-between px-4 lg:px-8">
                    <div className="hidden md:flex items-center gap-3">
                         ...
                         <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Phase {currentStep + 1}</span>
                    </div>
                    <div className="flex items-center gap-3 ml-auto">
                        <button onClick={handlePrevious} className="px-8 py-4 rounded-2xl bg-slate-100 text-slate-500 font-black text-[11px] uppercase tracking-widest active:scale-95 transition-all"><ArrowLeft size={16} /></button>
                        <button onClick={handleNext} disabled={isSaving} className={`px-12 py-4 rounded-2xl font-black text-[11px] uppercase tracking-[0.15em] text-white shadow-2xl transition-all active:scale-95 ${isLastStep ? 'bg-slate-900 shadow-slate-300' : 'bg-emerald-600 shadow-emerald-200 hover:bg-emerald-700'}`}>
                            {isSaving ? <Loader2 size={16} className="animate-spin" /> : (<span>{isLastStep ? "Complete & Done" : "Save & Continue"}</span>)}
                        </button>
                    </div>
                </div>
            </footer>
