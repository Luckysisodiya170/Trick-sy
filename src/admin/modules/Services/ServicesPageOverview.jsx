...
import { 
  ...
  Wrench, ArrowRight, Plus, Search, X, Box, Trash2, Edit3,
  Save, RotateCcw, ShieldCheck, Zap, Star, LayoutTemplate,
...
const ServicesPageOverview = () => {
  ...
  const [services, setServices] = useState(() => {
    const saved = localStorage.getItem('tricksy_services_manager');
    return saved ? JSON.parse(saved) : defaultServices;
  });

  useEffect(() => {
    localStorage.setItem('tricksy_services_manager', JSON.stringify(services));
  }, [services]);
  ...
  const handleDeleteService = (id, e) => {
    ...
    if(window.confirm("Delete this service permanently?")) {
      setServices(services.filter(sec => sec.id !== id));
    }
  };

  const handleRestoreDefaults = () => {
    if(window.confirm("Restore default Services?")) {
  ...
  const openCreateModal = () => {
    setEditingId(null);
    setFormData({ name: '', path: '', status: 'Draft', theme: 'indigo', iconKey: 'sparkles' });
    setIsModalOpen(true);
  };
  ...
  const handleSaveService = (e) => {
    ...
    const finalPath = formData.path.trim() || `/admin/pages/services/${formData.name.toLowerCase().replace(/\s+/g, '-')}`;
    ...
    if (editingId) {
      setServices(services.map(sec => sec.id === editingId ? { ...sec, ...serviceData } : sec));
    } else {
      setServices([...services, { id: `srv-custom-${Date.now()}`, ...serviceData }]);
    }
    ...
    <div className="min-h-screen bg-[#FDFDFD] p-6 lg:p-12 font-sans relative">
      ...
      <div className="w-full mb-10 md:mb-12 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-8">
        <div className="space-y-1">
          ...
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest italic">{services.length} Active Services</p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
             ...
             <input 
               type="text" placeholder="Find service..." 
               className="pl-11 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl w-full md:w-[220px] outline-none focus:ring-2 ring-indigo-500/20 focus:border-indigo-400 shadow-sm font-bold text-xs"
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
            ...
            <div key={sec.id} onClick={() => navigate(sec.path)} className="group relative bg-white p-4 md:p-5 rounded-[1.5rem] border border-slate-100 shadow-sm hover:border-indigo-500/20 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center gap-4 cursor-pointer overflow-hidden min-h-[90px] animate-in fade-in zoom-in-95">
              ...
              <div className="relative shrink-0 w-8 h-8 flex items-center justify-end">
                ...
                <div className="absolute right-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0">
                  ...
                  <button onClick={(e) => handleDeleteService(sec.id, e)} className="p-2 text-slate-400 bg-slate-50 hover:bg-rose-50 hover:text-rose-500 rounded-xl"><Trash2 size={14} /></button>
                </div>
              </div>
            </div>
          );
        ...
        <div onClick={openCreateModal} className="group p-4 md:p-5 rounded-[1.5rem] border-2 border-dashed border-slate-200 flex items-center gap-4 text-slate-400 hover:border-indigo-400 hover:text-indigo-500 hover:bg-indigo-50/50 transition-all cursor-pointer min-h-[90px] active:scale-95">
          ...
          <div className="flex-1"><h3 className="text-[14px] font-bold text-slate-700 group-hover:text-indigo-600">Add New Service</h3></div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl relative z-10 border border-slate-100 overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-300">
            <div className="px-8 pt-8 pb-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
              <div>
                <h2 className="text-2xl font-black text-slate-900">{editingId ? 'Edit Service' : 'Create New Service'}</h2>
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">Configure your service block</p>
              </div>
              ...
              <form onSubmit={handleSaveService} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Service Name</label>
                    <input required autoFocus value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-2 ring-indigo-100 transition-all" placeholder="e.g. Pest Control" />
                  </div>
                  ...
                  <div className="md:col-span-7">
                    ...
                    <div className="grid grid-cols-6 gap-2 bg-slate-50 p-4 rounded-[1.5rem] border border-slate-100">
                      {Object.keys(iconLibrary).map((key) => {
                        ...
                        return (
                          <button key={key} type="button" onClick={() => setFormData({...formData, iconKey: key})} className={`aspect-square rounded-xl flex items-center justify-center transition-all ${formData.iconKey === key ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-slate-400 border border-slate-100'}`}>
                            <IconComponent size={20} />
                          </button>
                        );
                      })}
            ...
            <div className="px-8 py-5 border-t border-slate-50 flex justify-end gap-4 bg-white">
              ...
              <button onClick={handleSaveService} className="px-8 py-3 rounded-xl text-xs font-black text-white bg-indigo-600 hover:bg-indigo-700 shadow-lg flex items-center gap-2 transition-all">
