...
import { 
  ...
  CreditCard, MessageCircleQuestion, ShieldCheck, Sparkles, 
  Plus, X, Box, Trash2, Edit3, ArrowRight, Save, RotateCcw, 
  Settings2, Wrench, Zap, Droplets, LayoutGrid, Scissors, Ruler
} from 'lucide-react';
...
const TechnicalPageOverview = () => {
  ...
  const [sections, setSections] = useState(() => {
    const saved = localStorage.getItem('tricksy_technical_modules');
    return saved ? JSON.parse(saved) : defaultSections;
  });

  useEffect(() => {
    localStorage.setItem('tricksy_technical_modules', JSON.stringify(sections));
  }, [sections]);
  ...
  const handleRestoreDefaults = () => {
    if(window.confirm("Restore default Technical sections?")) {
  ...
  const openCreateModal = () => {
    setEditingId(null);
    setFormData({ name: '', path: '', status: 'Draft', theme: 'indigo', iconKey: 'wrench' });
    setIsModalOpen(true);
  };
  ...
  const handleSaveModule = (e) => {
    ...
    const finalPath = formData.path.trim() || `/admin/pages/technical/${formData.name.toLowerCase().replace(/\s+/g, '-')}`;
    const moduleData = {
      name: formData.name, 
    ...
    if (editingId) {
      ...
      setSections([...sections, { id: `tech-custom-${Date.now()}`, ...moduleData }]);
    }
    ...
    <div className="min-h-screen bg-[#FDFDFD] p-6 lg:p-12 font-sans relative">
      ...
      <div className="w-full mb-10 md:mb-12 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-8">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Technical Services</h1>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button className="px-6 py-2.5 text-xs font-bold text-white bg-slate-900 rounded-xl hover:shadow-xl transition-all active:scale-95 flex items-center gap-2">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
            ...
            <div key={sec.id} onClick={() => navigate(sec.path)} className="group relative bg-white p-4 md:p-5 rounded-[1.5rem] border border-slate-100 shadow-sm hover:border-indigo-500/20 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center gap-4 cursor-pointer overflow-hidden min-h-[90px] animate-in fade-in zoom-in-95">
              ...
              <div className="flex-1 min-w-0 pr-1">
                <div className="flex flex-wrap items-center gap-2 mb-0.5">
                  <h3 className="text-[14px] md:text-[15px] font-bold text-slate-800 group-hover:text-indigo-600 transition-colors leading-tight">
                    {sec.name}
                  </h3>
        ...
        <div onClick={openCreateModal} className="group p-4 md:p-5 rounded-[1.5rem] border-2 border-dashed border-slate-200 flex items-center gap-4 text-slate-400 hover:border-indigo-400 hover:text-indigo-500 hover:bg-indigo-50/50 transition-all cursor-pointer min-h-[90px] active:scale-95">
          ...
          <div className="flex-1"><h3 className="text-[14px] font-bold text-slate-700 group-hover:text-indigo-600">Add Section</h3></div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl relative z-10 border border-slate-100 overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-300">
            <div className="px-8 pt-8 pb-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
              <div>
                <h2 className="text-2xl font-black text-slate-900">{editingId ? 'Edit Tech Section' : 'Create Custom Module'}</h2>
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">Configure your technical block</p>
              </div>
              ...
              <form onSubmit={handleSaveModule} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-2">
                    ...
                    <input required autoFocus value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-2 ring-indigo-100 transition-all" placeholder="e.g. Expertise Domains" />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pt-4 border-t border-slate-50">
                  ...
                  <div className="md:col-span-7">
                    ...
                    <div className="grid grid-cols-6 gap-2 bg-slate-50 p-4 rounded-[1.5rem] border border-slate-100 h-[160px] overflow-y-auto custom-scrollbar">
            ...
            <div className="px-8 py-5 border-t border-slate-50 flex justify-end gap-4 bg-white">
              ...
              <button onClick={handleSaveModule} className="px-8 py-3 rounded-xl text-xs font-black text-white bg-indigo-600 hover:bg-indigo-700 shadow-lg flex items-center gap-2 transition-all">
