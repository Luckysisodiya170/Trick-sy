...
const TestHeroEditor = () => {
    ...
    const [heroData, setHeroData] = useState({
        ...
        titleLine3: 'Services',
        description: 'We provide top-notch cleaning and maintenance services to keep your spaces spotless and well-maintained. Our team of experts is dedicated to delivering exceptional results with attention to detail and customer satisfaction.',
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] font-sans selection:bg-indigo-100 pb-20">
            ...
            <nav className="sticky top-0 z-[20] bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 py-3 flex items-center justify-between shadow-sm">
                ...
                <div className="w-1/4 sm:w-1/3 flex justify-end">
                    <button
                        ...
                        className="bg-slate-900 text-white px-4 sm:px-8 py-2.5 rounded-full font-extrabold text-[10px] sm:text-xs flex items-center gap-2 shadow-lg hover:bg-indigo-600 transition-all hover:-translate-y-0.5"
                    >
            ...
            <div className={`mx-auto transition-all duration-500 ${viewMode === 'split'
                ...
                {(viewMode === 'edit' || viewMode === 'split') && (
                    <div className={`${viewMode === 'split' ? 'lg:col-span-5' : ''} space-y-8 animate-in fade-in zoom-in-95 duration-300`}>
                            ...
                            <div className="space-y-5">
                                <div onFocus={() => setActiveField('badge')}>
                                    ...
                                    <input
                                        ...
                                        className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-sm focus:bg-white focus:border-indigo-400 outline-none transition-all"
                                    />
                                </div>
                                ...
                                <div className={`grid grid-cols-1 ${viewMode === 'split' ? 'xl:grid-cols-2' : 'md:grid-cols-2'} gap-5`} onFocus={() => setActiveField('title')}>
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <input value={heroData.titleLine1} onChange={(e) => setHeroData({ ...heroData, titleLine1: e.target.value })} placeholder="Professional" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-sm outline-none focus:bg-white focus:border-indigo-400 transition-all" />
                                        </div>
                                        <div>
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Heading Line 3</label>
                                            <input value={heroData.titleLine3} onChange={(e) => setHeroData({ ...heroData, titleLine3: e.target.value })} placeholder="Services" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-sm outline-none focus:bg-white focus:border-indigo-400 transition-all" />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black text-indigo-500 uppercase tracking-widest ml-1 mb-2 block">Highlight Text (Color)</label>
                                        <textarea
                                            ...
                                            className="w-full h-[124px] p-4 bg-indigo-50/50 border border-indigo-200 rounded-xl font-black text-indigo-700 outline-none focus:bg-white focus:border-indigo-400 resize-none transition-all"
                                        />
                                    </div>
                                </div>
                                ...
                                <div onFocus={() => setActiveField('desc')}>
                                    ...
                                    <textarea
                                        ...
                                        className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl text-sm font-medium leading-relaxed outline-none focus:bg-white focus:border-indigo-400 transition-all resize-none"
                                    />
                                </div>
                            </div>
                        </section>
                        ...
                        <div className={`grid grid-cols-1 ${viewMode === 'edit' ? 'md:grid-cols-2' : ''} gap-8`}>
                            ...
                            <section className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm">
                                <h3 className="font-black text-slate-900 mb-6 flex items-center gap-2 text-sm uppercase tracking-tight border-b border-slate-100 pb-4">
                                    <MousePointer2 size={16} className="text-indigo-500" /> Action Buttons
                                </h3>
                                <div className={`grid ${viewMode === 'split' ? 'grid-cols-1 gap-4' : 'grid-cols-1 sm:grid-cols-2 gap-5'}`} onFocus={() => setActiveField('buttons')}>
                                    <div>
                                        ...
                                        <input value={heroData.primaryBtnText} onChange={(e) => setHeroData({ ...heroData, primaryBtnText: e.target.value })} placeholder="Book Service" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-sm outline-none focus:bg-white focus:border-indigo-400 transition-all" />
                                    </div>
                                    <div>
                                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Secondary Button</label>
                                        <input value={heroData.secondaryBtnText} onChange={(e) => setHeroData({ ...heroData, secondaryBtnText: e.target.value })} placeholder="Get Free Quote" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-sm outline-none focus:bg-white focus:border-indigo-400 transition-all" />
                                    </div>
                                </div>
                            </section>
                            ...
                            <section className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm flex flex-col">
                                <h3 className="font-black text-slate-900 mb-6 flex items-center gap-2 text-sm uppercase tracking-tight border-b border-slate-100 pb-4">
                                    <ImageIcon size={16} className="text-indigo-500" /> Hero Media
                                </h3>
                                ...
                                <div
                                    ...
                                    className="flex-1 w-full border-2 border-dashed border-slate-200 bg-slate-50 rounded-2xl flex flex-col items-center justify-center p-6 group hover:bg-indigo-50 hover:border-indigo-200 transition-all cursor-pointer relative overflow-hidden min-h-[200px]"
                                >
                                    {heroData.mainImage ? (
                                        <div className="text-center">
                                            ...
                                            <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Change Image</p>
                                        </div>
                                    ) : (
                                        <>
                                            <div className="w-14 h-14 rounded-full bg-white shadow-lg flex items-center justify-center text-slate-300 group-hover:scale-110 group-hover:text-indigo-500 transition-all">
                                                <Upload size={20} />
                                            </div>
                    ...
                    <div className={`${viewMode === 'split' ? 'lg:col-span-7 lg:sticky lg:top-24 h-fit' : ''} animate-in fade-in zoom-in-95 duration-500`}>
                        <div className="w-full bg-white rounded-3xl sm:rounded-[3rem] border-4 sm:border-[12px] border-slate-900 shadow-2xl overflow-hidden relative">
                            ...
                            <div className="relative overflow-y-auto max-h-[80vh] custom-scrollbar bg-slate-50">
                                ...
                                <div className={`relative bg-gradient-to-br from-indigo-50 via-white to-indigo-50/40 min-h-[500px] sm:min-h-[600px] overflow-hidden flex items-center ${viewMode === 'split' ? 'py-12 px-6 lg:px-8' : 'py-16 px-6 sm:px-12 lg:px-20'} group`}>

                                    {/* Background Blurs */}
                                    <div className="absolute -top-20 -left-20 w-[200px] sm:w-[300px] h-[200px] sm:h-[300px] bg-indigo-100/60 rounded-full blur-[80px] pointer-events-none" />
                                    <div className="absolute -bottom-20 -right-20 w-[300px] sm:w-[400px] h-[300px] sm:h-[400px] bg-emerald-100/50 rounded-full blur-[100px] pointer-events-none" />
                                    ...
                                    <div className={`grid grid-cols-1 lg:grid-cols-12 w-full max-w-[1400px] mx-auto items-center gap-10 lg:gap-8 relative z-10`}>
                                        ...
                                        <div className={`col-span-1 lg:col-span-6 space-y-5 sm:space-y-6 ${viewMode === 'split' ? 'text-center' : 'text-center lg:text-left'}`}>
                                            <div className={`inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-white border border-indigo-100 shadow-sm transition-all duration-500 hover:scale-105 ${activeField === 'badge' ? 'ring-2 ring-indigo-400' : ''}`}>
                                                <Star className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-indigo-500 text-indigo-500" />
                                                <span className="text-[11px] sm:text-xs font-bold text-indigo-700">{heroData.badgeText || "Badge Text"}</span>
                                            </div>
                                            ...
                                            <div className={`flex flex-row items-center justify-center ${viewMode === 'split' ? '' : 'lg:justify-start'} gap-3 sm:gap-4 pt-2 sm:pt-4 transition-transform duration-500 ${activeField === 'buttons' ? 'scale-105' : ''}`}>
                                                <button className="bg-emerald-600 hover:bg-green-500 text-white px-4 sm:px-8 py-3 sm:py-3.5 rounded-xl font-bold text-[11px] sm:text-sm flex items-center gap-1.5 sm:gap-2 transition-all shadow-lg hover:shadow-indigo-500/30 hover:-translate-y-1 whitespace-nowrap">
                                        ...
                                        <div className="col-span-1 lg:col-span-6 relative flex justify-center mt-6 lg:mt-0">
                                            <div className={`relative aspect-square w-full max-w-[280px] sm:max-w-[360px] ${viewMode === 'split' ? 'xl:max-w-[380px]' : 'xl:max-w-[480px]'}`}>
                                                <div className="absolute inset-0 bg-indigo-100 rounded-full opacity-40 scale-[1.05] animate-pulse" />

                                                <div className={`absolute inset-0 rounded-full border-[6px] sm:border-[10px] border-white shadow-2xl overflow-hidden bg-white z-10 transition-all duration-700 hover:scale-[1.02] ${activeField === 'media' ? 'scale-[1.03] ring-4 ring-indigo-200' : ''}`}>
                                                    {heroData.mainImage ? (
                                                ...
                                                {/* Quality Badge */}
                                                <div className="absolute top-2 sm:top-10 -left-2 sm:-left-6 lg:-left-10 bg-white/95 backdrop-blur-sm px-3 sm:px-5 py-2.5 sm:py-4 rounded-xl sm:rounded-2xl shadow-xl flex items-center gap-2 sm:gap-3 animate-bounce border border-white z-20">
                                                ...
                                                {/* Map Badge */}
                                                <div className="absolute bottom-2 sm:bottom-10 -right-2 sm:-right-6 lg:-right-8 bg-white/95 backdrop-blur-sm px-3 sm:px-5 py-2.5 sm:py-4 rounded-xl sm:rounded-2xl shadow-xl flex items-center gap-2 sm:gap-3 border border-white hover:-translate-y-2 transition-transform duration-300 z-20">
