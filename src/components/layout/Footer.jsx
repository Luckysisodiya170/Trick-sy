...
const Footer = ({ footerData }) => {
  ...
  const content = { ...defaultContent, ...footerData };
  ...
  return (
    <footer className="bg-slate-900 text-slate-300 pt-12 lg:pt-16 pb-8 font-sans border-t-[4px] border-emerald-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-8 xl:gap-12 mb-12 lg:mb-16">
          ...
          <div className="sm:col-span-2 lg:col-span-4 space-y-5 lg:space-y-6">
            ...
            {/* <div className="flex gap-3 pt-2">
              ...
              <a href={content.socialLinks?.twitter || '#'} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-all hover:-translate-y-1">
                <Twitter className="w-4 h-4" />
              </a>
              <a href={content.socialLinks?.instagram || '#'} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-all hover:-translate-y-1">
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-3 lg:space-y-4">
              {content.services?.map((service, idx) => (
                <li key={idx}>
                  <Link to={service?.path || '/services'} className="text-sm hover:text-emerald-400 transition-colors flex items-center gap-2 group">
                    <ArrowRight className="w-3.5 h-3.5 text-emerald-500 transition-transform group-hover:translate-x-1" /> 
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="space-y-4">
              ...
              <li className="flex items-center gap-3 group">
                ...
                <a href={`tel:${content.phone?.replace(/[^0-9+]/g, '')}`} className="text-sm font-medium group-hover:text-emerald-400 transition-colors">
                  {content.phone}
                </a>
              </li>
              <li className="flex items-center gap-3 group">
                <Mail className="w-5 h-5 text-emerald-500 shrink-0" />
                <a href={`mailto:${content.email}`} className="text-sm group-hover:text-emerald-400 transition-colors">
                  {content.email}
                </a>
              </li>
            ...
            <form className="space-y-3" onSubmit={handleNewsletterSubmit}>
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-slate-800 border border-slate-700 text-white pl-10 pr-4 py-3.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all text-sm"
                  required
                />
              </div>
              <button type="submit" className="w-full bg-emerald-500 hover:bg-emerald-400 text-white font-semibold py-3.5 rounded-xl transition-all shadow-lg hover:-translate-y-0.5">
                Subscribe Now
              </button>
            </form>
          </div>
