...
const AboutMission = ({ 
  ...
  const stats = statsData?.length ? statsData : defaultStats;
          ...
          <div className="w-full lg:w-1/2 relative">
            ...
            <div className="absolute -bottom-8 -right-8 hidden md:flex flex-col bg-zinc-950 text-white p-8 rounded-[2rem] border-4 border-zinc-800 z-20 w-[260px] transform hover:-translate-y-2 transition-transform duration-300">
              ...
              <p className="text-zinc-400 font-bold text-sm leading-snug mt-2 uppercase tracking-widest">
                Satisfaction <br/><span className="text-indigo-500">Guaranteed</span>
              </p>
            </div>
          </div>
          ...
          <div className="w-full lg:w-1/2">
            ...
            <div className="grid grid-cols-2 gap-4 lg:gap-6 mt-8">
              {stats.map((stat, idx) => (
                ...
                >
                  <div className="w-12 h-12 rounded-xl bg-zinc-100 group-hover:bg-indigo-500 flex items-center justify-center text-zinc-950 group-hover:text-white transition-colors duration-300 mb-4 border border-zinc-200 group-hover:border-indigo-500">
                    {stat.icon}
                  </div>
                  <p className="text-4xl font-black text-zinc-950 mb-1 leading-none">{stat.value}</p>
                  <p className="text-[11px] font-black text-zinc-500 uppercase tracking-widest mt-2 group-hover:text-indigo-600 transition-colors">
                    {stat.label}
                  </p>
                </div>
              ))}
