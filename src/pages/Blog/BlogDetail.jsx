...
const BlogDetail = ({ previewData }) => {
  ...
  useEffect(() => {
    ...
    const fetchSinglePost = async () => {
      ...
      try {
        ...
        setPost(fallback);
        if (fallback) {
          setRelatedPosts(dummyPosts.filter(p => p.category === fallback.category && p.slug !== slug).slice(0, 2));
        }
      } finally {
        setLoading(false);
      }
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-white"><Loader2 className="w-10 h-10 animate-spin text-emerald-500" /></div>;
    ...
    <div className="bg-white min-h-screen pt-24 pb-20">
      ...
      <article className="w-full max-w-[900px] mx-auto px-6">
        <div className="flex items-center justify-between mb-10">
          ...
          <span className="px-4 py-1.5 bg-zinc-950 text-white font-black text-[10px] uppercase tracking-widest rounded-full">{post.category}</span>
        </div>

        <h1 className="text-4xl md:text-6xl font-black text-zinc-950 leading-[1.05] tracking-tighter mb-8">{post.title}</h1>
            ...
            <div key={index}>
              ...
              {block.type === 'image' && (
                <figure className="my-10 rounded-3xl overflow-hidden border border-zinc-100 shadow-lg">
                  <img src={block.value} alt="article" className="w-full" />
                </figure>
              )}
            </div>
          ))}
